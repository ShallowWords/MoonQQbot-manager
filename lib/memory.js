...
// ------------------------------------------------------------------
// 分层记忆（重写版）：
//   种子文件   persona.md / traits.md        —— 人格种子，不再全文注入，蒸馏为 L0 核心卡
//              plot.md / content.md          —— 大文件，不再全文注入，注入其分段摘要（L3）
//   L0 核心卡  persona_core.json             —— 种子 + 近期经历蒸馏而成，每轮注入（小而稳）
//   L1 事件流  events.jsonl                  —— 结构化经历事件（重要性过滤 + 滚动压缩）
//              events_summary.md             —— 老事件压缩摘要（常驻注入）
//              events_archive.md             —— 压缩前的原始事件归档（不注入）
//   L3 摘要    plot_summary.json / content_summary.json —— 大文件分段摘要（RAG 前的过渡）
...
// 记忆文件定义（顺序即面板列表顺序；persona/traits/plot/content 已由分层记忆接管注入方式）
const FILE_TYPES = [
  { key: 'persona',     name: '人格',      desc: '人格种子：蒸馏为人格核心卡后注入对话' },
  { key: 'plot',        name: '剧情',      desc: '剧情背景/世界观：注入其分段摘要' },
  { key: 'content',     name: '内容',      desc: '知识/资料储备：注入其分段摘要' },
  { key: 'traits',      name: '特征',      desc: '特征种子：并入人格核心卡蒸馏' },
];
...
function botDir(botId) {
  ...
  // 首次创建目录时初始化预设模板；之后以磁盘为准，删除后不再自动重建
  if (!existed) {
    for (const t of FILE_TYPES) {
      const def = DEFAULTS[t.key];
      if (def !== undefined) fs.writeFileSync(path.join(dir, `${t.key}.md`), def, 'utf8');
    }
  }
  return dir;
}
...
function defaultTier(key) {
  if (key === 'persona' || key === 'traits') return 1;
  return 2;
}
...
// 冷记忆文件清单（启用的 tier3，供 system 索引与 recall_memory 工具）
function getColdFiles(botId) {
  const meta = getMeta(botId);
  return getFileOrder(botId)
    .filter((k) => !MIGRATED_KEYS.has(k) && k !== 'persona' && k !== 'traits' && GENERATED_KEYS.has(k) === false)
    .filter((k) => getFileEnabled(botId, k) && getFileTier(botId, k) === 3)
    .map((k) => {
      const preset = FILE_TYPES.find((f) => f.key === k);
      return { key: k, name: preset ? preset.name : k, desc: meta.files[k]?.desc || '' };
    });
}
...
// 删除记忆文件（删除即真正消失，不会自动重建；同时清理 _meta.json 中的启用状态）
...
// 返回 [{ key, name, desc, content, enabled }]
function getMemoryFiles(botId) {
  ...
  return getFileOrder(botId).map((key) => {
    ...
    return {
      ...
      // 备注优先使用面板中自定义的，否则用预设描述/默认文案
      desc: meta.files[key]?.desc || (preset ? preset.desc : '自定义记忆文件（内容将随 system prompt 提供给 AI）'),
...
async function compactEventsIfNeeded(botId, chatFn) {
  if (!chatFn || _compacting.has(botId)) return { ok: true, skipped: true };
  const rows = readEvents(botId);
  if (rows.length <= EVENT_THRESHOLD) return { ok: true, skipped: true };
  _compacting.add(botId);
  try {
    const old = rows.slice(0, EVENT_COMPACT_OLD);
    ...
    fs.writeFileSync(eventsFile(botId), rows.slice(EVENT_COMPACT_OLD).map((r) => JSON.stringify(r)).join('\n') + '\n', 'utf8');
...
// 种子指纹：persona + traits 任一被修改即视为过期
function seedHash(botId) {
  return md5(readMemoryFile(botId, 'persona') + '\x00' + readMemoryFile(botId, 'traits'));
}
...
// 种子是否为空或还是初始模板（模板不值得蒸馏）
function isSeedEmpty(botId) {
  const p = readMemoryFile(botId, 'persona').trim();
  const t = readMemoryFile(botId, 'traits').trim();
  return (!p || p === (DEFAULTS.persona || '').trim()) && (!t || t === (DEFAULTS.traits || '').trim());
}
...
// 蒸馏人格核心卡：种子(全文) + 近期事件 + 近期对话 → 固定 schema 的小核心卡
async function distillPersonaCore(botId, chatFn) {
  if (!chatFn) return { ok: false, err: '无可用模型' };
  if (isSeedEmpty(botId)) return { ok: false, err: '人格种子为空（先填写人格/特征）' };
  const seed = [readMemoryFile(botId, 'persona').trim(), readMemoryFile(botId, 'traits').trim()]
    .filter(Boolean).join('\n\n');
  ...
  const dial = getRecentSessions(botId, 30).slice(-16)
    ...
    .join('\n');
  const sys = '你是人格蒸馏器。依据「人格种子」（原始人设与特征）与「近期经历」（关键事件与对话摘录），' +
    '提炼一份精简、稳定、可直接指导对话的人格核心卡。字段要求（全部为字符串）：' +
    'identity 身份与基调（≤80字）；tone 说话风格，用指令式描述（≤80字）；boundaries 不会做/禁忌/底线（≤60字）；' +
    'relationship_state 当前与用户的关系阶段（≤60字）；evolved_notes 种子之外从经历中沉淀的性格演化，没有写"暂无"（≤80字）。' +
...
// 种子（persona/traits）变更仍会触发重蒸馏并覆盖手动编辑——种子是人格的最终权威
...
async function summarizeSource(botId, key, chatFn) {
  if (!chatFn) return { ok: false, err: '无可用模型' };
  const src = readMemoryFile(botId, key).trim();
  if (!src || src === (DEFAULTS[key] || '').trim()) return { ok: false, err: `${key} 为空，跳过` };
...
async function maybeDistill(botId, chatFn) {
  if (isSeedEmpty(botId)) return { ok: false, skipped: true };
...
// 摘要按需生成：所有启用的 tier2 文件（种子变更后自动重生成）
async function ensureSummaries(botId, chatFn) {
  ...
  for (const key of getFileOrder(botId)) {
    ...
    if (key === 'persona' || key === 'traits') continue;
    if (!getFileEnabled(botId, key)) continue;
    if (getFileTier(botId, key) !== 2) continue;
    if (isSummaryFresh(botId, key)) continue;
    try { out.push({ key, ...(await summarizeSource(botId, key, chatFn)) }); }
    catch (e) { out.push({ key, ok: false, err: e.message }); }
  }
  return out;
}
...
function getMemState(botId) {
  ...
  return {
    ...
    summaries: Object.fromEntries(['plot', 'content'].map((k) => {
      const s = readSummary(botId, k);
      return [k, s ? { ts: s.ts, fresh: s.srcHash === md5(readMemoryFile(botId, k)), sections: s.sections } : null];
    })),
  };
}
...
function buildSystemPrompt(botId, opts = {}) {
  ...
  // L0 人格核心卡；缺失/过期 → tier1 的 persona/traits 种子全文回退（旧行为），后台蒸馏完成后自动切换
  const core = readPersonaCore(botId);
  if (core && isCoreFresh(botId)) {
    head.push('【人格核心】\n' + formatCore(core.core));
  } else {
    for (const k of order) {
      if (k !== 'persona' && k !== 'traits') continue;
      if (!getFileEnabled(botId, k) || getFileTier(botId, k) !== 1) continue;
      const c = readFile(k).trim();
      if (c) head.push(`【${k === 'persona' ? '人格' : '特征'}】\n${c}`);
    }
  }
  // 中层（tier2）+ 头部（tier1）
  for (const key of order) {
    if (MIGRATED_KEYS.has(key) || GENERATED_KEYS.has(key)) continue;
    if (key === 'persona' || key === 'traits') continue; // 由人格核心卡/种子回退逻辑接管
    if (!getFileEnabled(botId, key)) continue;
    ...
    const c = readFile(key).trim();
    if (c && estTokens(c) <= 1500) mid.push(`【${label}】\n${c}`);
  }
...
module.exports = {
  ...
  readPersonaCore, isCoreFresh, distillPersonaCore,
  summarizeSource, isSummaryFresh, maintain, getMemState,
