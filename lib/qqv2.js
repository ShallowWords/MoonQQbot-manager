...
async function sendReply(appId, appSecret, sandbox, scene, targetId, content, msgId) {
  const token = await getAccessToken(appId, appSecret);
  const base = sandbox ? API_SANDBOX : API_PROD;
  ...
  const res = await fetch(base + path, {
    ...
    body: JSON.stringify({ msg_type: 0, content, msg_id: msgId || '', msg_seq: ++_msgSeq }),
  });
  const data = await res.json().catch(() => ({}));
  if (!res.ok) {
    throw new Error(`发送消息失败 (HTTP ${res.status}): ${data.message || JSON.stringify(data).slice(0, 300)}`);
  }
...
module.exports = { getAccessToken, getGatewayUrl, sendReply, startBot, normalizeEvent, buildIntentMask, INTENT_BITS };
