...
check("node_modules 已安装", hasNm, hasNm ? "" : "", "在 desktop 目录运行 npm install");

// 前端资源：本项目没有构建步骤，tauri.conf.json 的 build.frontendDist
// 直接指向仓库里现成的静态目录（../../public）。这里按配置实际指向去检查，
// 而不是假设存在 dist/（原模板是 Vite 流程，本仓库已剥离）。
let frontendDir = "";
let frontendDistRaw = "";
try {
  ...
  frontendDistRaw = conf.build?.frontendDist || "";
  if (frontendDistRaw) {
    frontendDir = path.resolve(path.join(ROOT, "src-tauri"), frontendDistRaw);
  }
} catch {
  /* 下面会用空值报错 */
}
...
check(
  "前端资源（frontendDist 指向的目录）",
  hasFrontend,
  hasFrontend ? frontendDistRaw : "",
  frontendDistRaw
    ? `检查 ${frontendDir} 里是否有 index.html`
    : "tauri.conf.json 未配置 build.frontendDist",
);
...
const blockers = [];
if (!hasNm) blockers.push("Tauri CLI 未安装  ->  在 desktop/ 目录 npm install");
if (!hasFrontend)
  blockers.push(
    frontendDistRaw
      ? `前端资源缺失（frontendDist = ${frontendDistRaw}）`
      : "tauri.conf.json 未配置 build.frontendDist",
  );
